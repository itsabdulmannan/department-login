...
const AddCheifEditorsModal = ({ isOpen, onClose, fetchSectionHeads }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const { createSectionHead, loading } = useHooks();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setTimeout(() => {
        toast.success("Section Head added successfully!");
      }, 500); // Delayed toast
      ...
      setTimeout(() => {
        toast.error("Failed to add Section Head. Please try again.");
      }, 500); // Delayed toast
    } finally {
  ...
  return (
    <>
      <motion.div
        className="fixed inset-0 z-40 overflow-y-auto flex items-center justify-center bg-black bg-opacity-50"
      ...
      >
        <motion.div
          className="relative bg-white rounded-lg shadow-lg p-6 overflow-y-auto w-[80%] h-[80%] sm:w-[70%] sm:h-auto"
          // style={{
          //   width: "70%",
          //   height: "auto",
          // }}
