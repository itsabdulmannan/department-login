...
const ViewReviewedPaer = () => {
  const { state } = useLocation();
  const paperId = state?.paperId; 
  console.log(paperId, "Received Paper ID");

  const { fetchReviewedPapersById, loading, dataByID, error } = useHooks();

  useEffect(() => {
    if (paperId) {
      fetchReviewedPapersById(paperId);
    } else {
...
export default ViewReviewedPaer;
