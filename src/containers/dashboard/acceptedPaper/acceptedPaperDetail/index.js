...
const AcceptedPAperDetail = () => {
  const { state } = useLocation();
  const paperId = state?.paperId;

  const { fetchAcceptedPapersById, loading, dataByID, error } = useHooks();

  useEffect(() => {
    if (paperId) {
      fetchAcceptedPapersById(paperId);
    } else {
...
export default AcceptedPAperDetail;
